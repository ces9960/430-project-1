...
const compareFields = (filterField, classField) => (filterField === 'Any' || filterField === classField || !filterField);
...
const postCharacterResponse = (request, response, body) => {
  if (!body.Name || !body.Class || !body.characterDescription) {
    ...
    response.write(JSON.stringify({ id: users[body.Name].Name, message: 'Created Successfully' }));
    response.end();
  }
};

const getCharacters = (request, response, params, acceptedTypes, httpMethod) => {
  let responseXML = '';
  const userArray = Object.keys(users);
  for (let i = 0; i < userArray.length; i += 1) {
    responseXML += `
      <character>
        <name>${userArray[i].Name}</name>
        <class>${userArray[i].Class}</class>
        <description>${userArray[i].characterDescription}</description>
      </character>
    `;
  }
  if (httpMethod === 'HEAD') {
    if (acceptedTypes.includes('text/xml')) {
      ...
      response.writeHead(200, { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(JSON.stringify(users)) });
      response.end();
    }
...
module.exports.postCharacterResponse = postCharacterResponse;
